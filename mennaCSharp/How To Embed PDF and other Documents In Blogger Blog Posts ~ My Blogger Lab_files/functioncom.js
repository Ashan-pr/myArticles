...
if (typeof(Emo_List) == 'undefined') var Emo_List = ['O.O', 'https://lh6.googleusercontent.com/-wFaOATKR4SA/T30v4DhsyaI/AAAAAAAAB9Y/IUXospAJeOY/s68/andor.gif'];

function Bi_Online_Valid_Domain(url) {
    key = '.blogspot.';
    index = url.indexOf(key);
    if (index != -1) {
        url = url.substring(0, index + key.length) + 'com'
    }
    return url
}
function Bi_Online_Nake_Url(url) {
    ...
    url = Bi_Online_Valid_Domain(url);
    return escape(url)
}
